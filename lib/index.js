...
async function apply(ctx, config = {}) {
    ...
    // kimi's three built-in subagent types, self-contained: persona (distilled
    // from kimi-cli's own subagent prompts) and toolFilter (kimi-surface tool
    ...
    const KIMI_SUBAGENTS = {
      coder: {
        ...
        toolFilter: { allow: ['Shell', 'ReadFile', 'WriteFile', 'StrReplaceFile', 'Glob', 'Grep', 'ReadMediaFile', 'SearchWeb', 'FetchURL', 'SetTodoList', 'AskUserQuestion', 'TaskList', 'TaskOutput', 'TaskStop'] },
      },
      explore: {
        provider: 'kimi-explore',
        persona: 'You are the Kimi Code "explore" subagent: a fast, read-only codebase exploration specialist. You have NO file editing tools. Use Glob for file patterns, Grep for content search, ReadFile for known paths; run Shell ONLY for read-only commands (ls, git status, git log, git diff, find). Adapt search depth to the thoroughness requested by the caller (quick | medium | thorough). Prefer parallel tool calls. Report findings in a structured format with absolute paths.',
        toolFilter: { allow: ['Shell', 'ReadFile', 'Glob', 'Grep', 'ReadMediaFile', 'SearchWeb', 'FetchURL'] },
      },
      plan: {
        provider: 'kimi-plan',
        ...
        toolFilter: { allow: ['ReadFile', 'Glob', 'Grep', 'ReadMediaFile', 'SearchWeb', 'FetchURL'] },
      },
    }
    register(strDef({
      name: 'Agent',
      ...
      execute: async (args, exec) => {
        ...
        const spec = KIMI_SUBAGENTS[args.subagent_type || 'coder'] || KIMI_SUBAGENTS.coder
        const names = subagents.list()
        let providerName = names.indexOf(spec.provider) >= 0 ? spec.provider : (names.indexOf('spawn') >= 0 ? 'spawn' : null)
        ...
        const label = String(args.description || spec.provider).slice(0, 80)
        const request = {
          label,
          prompt: [{ type: 'text', text: String(args.prompt) }],
          parent: exec.agent,
          agentOptions: { provider: 'kimi-kernel', model: args.model || 'k3-256k' },
